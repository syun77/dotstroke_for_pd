...
ファイル操作は File メニューから行えます。macOS ではシステムメニューバーにも表示され、`Cmd+N` で新規作成、`Cmd+O` で JSON 読み込み、`Cmd+S` で保存、`Cmd+Shift+S` で Save As ができます。読み込み済みの JSON は `Cmd+S` で同じファイルへ直接保存され、Save As では保存先を選んで別ファイルとして保存できます。File > Open Recent から過去に開いたJSONを選択でき、エディタ起動時には最後に開いていたファイルを自動で開きます。履歴は最大10件までアプリ設定に保存されます。Lua のコピーはプレビュー下のボタン、または `Cmd+P` で実行できます。
